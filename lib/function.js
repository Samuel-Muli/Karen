...
const GIFBufferToVideoBuffer = async (image) => {
    ...
    await fs.writeFileSync(`./Karen/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./Karen/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./Karen/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./Karen/trash/${filename}.mp4`)
    Promise.all([unlink(`./Karen/trash/${filename}.mp4`), unlink(`./Karen/trash/${filename}.gif`)])
    return buffer5
}
